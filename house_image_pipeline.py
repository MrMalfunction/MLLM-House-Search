...
class HouseImagePipeline:
    ...
    def parse_image_filename(self, filename: str) -> tuple | None:
        """
        ...
        for house_id in sorted(all_house_ids):
            ...
            if house_metadata is None:
                ...
                house_metadata = HouseMetadata(
                    house_id=house_id, bedrooms=0, bathrooms=0, area=0, zipcode=0, price=0
                )
            ...
            house = House(house_id=house_id, metadata=house_metadata, images=house_images)
            houses.append(house)
...
def main():
    ...
    parser.add_argument(
        "--output", type=str, default="house_image_associations.json", help="Output JSON file path"
    )
    parser.add_argument("--stats", action="store_true", help="Print statistics about the dataset")
